...
namespace coder::dsp
{
	...
	void b_ColoredNoise::step( double varargout_1[ 4096 ], unsigned state[ 625 ] )
	{
		...
		for(int jj = 0; jj < 5; jj++)
		{
			...
			for(int k = 0; k < 4096; k++)
			{
				...
				{
					out_tmp = ( k + j ) + 1;
					out[ out_tmp ] += as * a[ j + 1 ];
				}
			}

			for(j = 0; j < 1; j++)
			{
				dv[ 0 ] += varargout_1[ 4094 ] * b[ 2 ];
			}

			for(j = 0; j < 2; j++)
			{
				dv[ j ] += varargout_1[ 4095 ] * b[ j + 1 ];
			}
	...
	void c_ColoredNoise::step( double varargout_1[ 4096 ], unsigned state[ 625 ] )
	{
		...
		{
			dv[ k ] = 0.0;
			out[ k ] = this->pFilterStates[ k ];
		}

		std::memset( &out[ 255 ], 0, 3841U * sizeof( double ) );
		for(k = 0; k < 256; k++)
		{
			int b_k;
			b_k = k + 1;
			for(j = b_k; j < 4097; j++)
			{
				out[ j - 1 ] += dv1[ k ] * varargout_1[ ( j - k ) - 1 ];
			}
		}
		...
		for(k = 0; k < 255; k++)
		{
			for(j = 0; j <= k; j++)
			{
				dv[ j ] += varargout_1[ k + 3841 ] * dv1[ ( j - k ) + 255 ];
			}
		}
